...
export function ContactModal({ open, contact, onClose }: Props) {
  ...
  const contactTasks = useMemo(() => tasks.filter((task) => task.contact_id === contact?.id), [tasks, contact?.id])
      ...
      <div className="grid gap-4">
        ...
        <section className="rounded-xl border border-[var(--border)] p-3">
          ...
          <div className="grid gap-2">
            {contactTasks.length === 0 && <p className="text-sm text-[var(--muted)]">No tasks linked to this contact.</p>}
            {contactTasks.map((task) => (
              <TaskCard
                ...
                onToggle={(item) => void updateTask(item.id, { status: item.status === 'done' ? 'toDo' : 'done' })}
              />
            ))}
          </div>
        </section>
