...
export function NurtureCard({
  ...
  return (
    <article
      className={`relative cursor-pointer rounded-[9px] border-[0.5px] bg-white p-3 transition hover:border-[rgba(143,167,144,0.35)] hover:bg-[#f8fdf8] hover:shadow-[0_2px_8px_rgba(0,0,0,0.06)] ${
        overdue
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[12.5px] font-medium leading-tight text-[var(--meeting)]">
            {contact.name}
          </p>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-1.5 text-[10.5px] text-[var(--muted)]">
        ...
        <span className="rounded bg-[#f5f3f0] px-2 py-1">
          Every {contact.nurture_frequency_days ?? '—'}d
        </span>
      </div>

      <p className="mt-2 text-[10.5px] text-[var(--muted)]">
        Last meeting: {dateLabel(contact.next_call_date)}
      </p>

      <details
        className="mt-3 rounded-[7px] bg-[#faf9f8] px-2 py-1.5 text-[11px] text-[var(--muted)]"
        onClick={(event) => event.stopPropagation()}
      >
        <summary className="cursor-pointer text-[var(--nurture)]">
          ✦ Context from last interaction
        </summary>
        <p className="mt-1 leading-relaxed">
          Generate context with Jamie to prep a thoughtful follow-up.
        </p>
      </details>
    </article>
  )
