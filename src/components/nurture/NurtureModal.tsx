...
export function NurtureModal({
  ...
  const visibleLogs = useMemo(() => logs.filter((entry) => !entry.archived), [logs])
  ...
  const handleSaveChanges = async () => {
    ...
    const nextDate = frequency
      ? isoFromDateInput(addDaysDateInput(frequency))
      : isoFromDateInput(nextNurtureDate)
    ...
    <Modal open={open} onClose={onClose} title={contact.name} maxWidthClassName="max-w-3xl">
      <div className="max-h-[76vh] overflow-y-auto pr-1">
        <div className="space-y-4">
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      <button
                        ...
                        aria-label={entry.collapsed ? 'Expand log notes' : 'Collapse log notes'}
                      >
                        {entry.collapsed ? '▸' : '▾'}
                      </button>
          ...
          <div className="sticky bottom-0 flex items-center justify-end gap-3 border-t border-[var(--border)] bg-white pt-4">
            <button
              type="button"
              onClick={onClose}
